...
public class DownloadHistoryManager {
    ...
    private static final Set<Long> monitoringDownloads = new HashSet<>();
    ...
    public static void monitorDownloadProgress(Context context, long downloadId, DownloadManager dm) {
        synchronized (monitoringDownloads) {
            if (monitoringDownloads.contains(downloadId)) {
                return;
            }
            monitoringDownloads.add(downloadId);
        }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                synchronized (monitoringDownloads) {
                    monitoringDownloads.remove(downloadId);
                }
            }
        }).start();
    }
