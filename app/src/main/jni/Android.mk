...
ifneq ($(wildcard $(LOCAL_PATH)/libs/$(TARGET_ARCH_ABI)/libz.a),)
    include $(CLEAR_VARS)
    LOCAL_MODULE := z
    ...
    include $(PREBUILT_STATIC_LIBRARY)
    Z_TYPE := static
else ifneq ($(wildcard $(LOCAL_PATH)/libs/$(TARGET_ARCH_ABI)/libz.so),)
    include $(CLEAR_VARS)
    LOCAL_MODULE := z
    ...
    $(warning [Android.mk] WARNING: zlib (libz.a / libz.so) not found for $(TARGET_ARCH_ABI) — will fallback to system -lz)
    Z_TYPE := none
endif
