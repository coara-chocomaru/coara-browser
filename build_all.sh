...
if [ ! -d "$ANDROID_NDK_ROOT" ]; then
  die "ANDROID_NDK_ROOT directory not found: $ANDROID_NDK_ROOT"
fi
...
if [ ! -d "$TOOLCHAIN_BIN" ]; then
  die "NDK toolchain bin not found at: $TOOLCHAIN_BIN"
fi
...
download_and_extract() {
  ...
  if [ ! -d "$targ" ]; then
    ...
    curl -L -f -o "$archive" "$url" || die "Failed to download $url"
    ...
    info "$targ already exists, skipping download."
  fi
}
...
if [ ! -d "$ZLIB_SRCDIR" ]; then
  download_and_extract "https://zlib.net/zlib-$ZLIB_VER.tar.gz" "$ZLIB_SRCDIR"
fi


LIBPNG_SRCDIR="$SRCDIR/libpng-$LIBPNG_VER"
if [ ! -d "$LIBPNG_SRCDIR" ]; then
  download_and_extract "https://download.sourceforge.net/libpng/libpng-$LIBPNG_VER.tar.gz" "$LIBPNG_SRCDIR"
fi

OPENSSL_SRCDIR="$SRCDIR/openssl-$OPENSSL_VER"
if [ ! -d "$OPENSSL_SRCDIR" ]; then
  download_and_extract "https://www.openssl.org/source/openssl-$OPENSSL_VER.tar.gz" "$OPENSSL_SRCDIR"
fi
...
for ABI in "${ABIS[@]}"; do
  ...
  OPENSSL_LIBS_FOUND=false

  find . -type f \( -name "libcrypto.so" -o -name "libcrypto.*.so" -o -name "libcrypto.a" \) -print -exec cp -av {} "$ABI_DIST/lib/" \; -exec sh -c 'OPENSSL_LIBS_FOUND=true' \; || true
